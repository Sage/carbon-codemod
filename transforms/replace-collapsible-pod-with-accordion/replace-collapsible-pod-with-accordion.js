...
export default function transformer(file, api) {
  ...
  function addAccordionImport(j, root) {
    ...
    accordionImport = j.importDeclaration(
      [j.importDefaultSpecifier(j.identifier(names.accordionComponent))],
  ...
  function addAccordion(j, podNodes) {
    podNodes.forEach((podNode) => {
      ...
      if (titleJSXAttributeProp.size()) {
        accordionAttributes.push(getTitleAttribute(titleJSXAttributeProp));
        ...
        if (typeOfSpreadAttributeObject === "Identifier") {
          ...
          spreadAttributeObject.forEach((attr) => {
            ...
            if (titleInDeclarationScope.size()) {
              accordionAttributes.push(
                getTitleAttribute(titleInDeclarationScope)
              );
          ...
          if (titleAsObjectProp.size()) {
            accordionAttributes.push(getTitleAttribute(titleAsObjectProp));
      ...
      const accordionOpeningTag = getAccordionOpeningTag(accordionAttributes);
  ...
  function getTitleAttribute(titleJSXAttributeProp) {
    const titlePropValue = titleJSXAttributeProp.get("value").value;
    const isValueLiteral = titlePropValue.type === "Literal";

    if (titlePropValue && titlePropValue.type === "Identifier") {
      return j.jsxAttribute(
        j.jsxIdentifier(names.titleProp),
        j.jsxExpressionContainer(j.jsxIdentifier(titlePropValue.name))
      );
    }
    ...
    return j.jsxAttribute(
      j.jsxIdentifier(names.titleProp),
      isValueLiteral
        ? j.stringLiteral(titlePropValue.value)
        : j.jsxExpressionContainer(
            j.jsxIdentifier(titlePropValue.expression.name)
          )
    );
  }
  ...
  function getExpandedAttribute(collapsedProp) {
    ...
    if (
      ...
      propValue.value.expression.type === "Literal"
    ) {
      return j.jsxAttribute(j.jsxIdentifier(names.expandedProp));
    }
